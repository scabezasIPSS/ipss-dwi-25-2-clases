                ...
                <table class="table table-striped mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ !$estado['activo'] ? 'table-secondary' : '' }}">
                            ...
                            <td>
                              @if($estado['activo'])
                                <form action="{{ route('backoffice.estadospago.down', $estado['id']) }}" method="POST" style="display:inline">
                                    @csrf
                                    <button type="submit" class="btn btn-sm btn-warning mb-1">↓ Desactivar</button>
                                </form>
                              @else
                                <form action="{{ route('backoffice.estadospago.up', $estado['id']) }}" method="POST" style="display:inline">
                                    @csrf
...
<div class="modal fade" id="modalCreateEstado" tabindex="-1" aria-labelledby="modalCreateEstadoLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <form action="{{ route('backoffice.estadospago.new') }}" method="POST">
        @csrf
